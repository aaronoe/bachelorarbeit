...
\subsubsection{An alternative characterisation of popular matchings}
...
\begin{theorem}\label{theorem-popular-4}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            \item if $f_j \leq c_j$, then $f(h_j) \subseteq M(h_j)$
            \item if $f_j > c_j$, then $|M(h_j)| = c_j$ and $M(h_j) \subseteq f(h_j)$
        \end{enumerate}
